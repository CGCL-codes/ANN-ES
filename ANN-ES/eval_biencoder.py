...
import common.nn_prediction as nnquery
import common.utils as utils
from common.zeshel_utils import WORLDS, load_entity_dict_zeshel, Stats
...
def encode_candidate(
    ...
    if is_zeshel:
        ...
        for src, cand_pool in candidate_pool.items():
            ...
            cand_tokens = np.array([np.array(item["ids"]) for item in cand_pool])
            cand_pool_encode = encode_candidate(
                reranker,
                cand_tokens, # cand_pool,
                encode_batch_size,
                silent,
                logger,
                is_zeshel=False,
            )
            cand_encode_dict[src_index] = cand_pool_encode
            src_index += 1
    ...
    for step, batch in enumerate(iter_):
        ...
        cand_encode = reranker(None, cands, None, True, False) # context_input, cand_input, label_input=None, encode_only=False, encode_context=True
...
def main(params):
    ...
    if cand_pool_path and os.path.exists(cand_pool_path):
        with open(cand_pool_path, "rb") as f:
            pkl_obj = pickle.load(f)
            candidate_pool = pkl_obj[0]
        #    candidate_pool, _, _ = pickle.load(f)
    else: 
    ...
    test_tensor_path = os.path.join(params["data_path"], "{}_tensor_data.pkl".format(params["mode"]))
    if os.path.exists(test_tensor_path):
        with open(test_tensor_path, "rb") as f:
            test_tensor_data = pickle.load(f)
    else:
        ...
        #     debug=params["debug"],
        # )
        _, test_tensor_data = data.process_mention_data_by_presumm(
            test_samples,
            tokenizer,
            params["max_context_length"],
            params["max_cand_length"],
            context_key=params["context_key"],
            silent=params["silent"],
            entity_tensor_dict=candidate_pool, 
            logger=logger,
            debug=params["debug"],
        )
    ...
    candidate_pool_1 = {}
    i = 0
    for src, item in candidate_pool.items():
        tensors = np.array([np.array(c["ids"]) for c in item])
        candidate_pool_1[i] = tensors
        i += 1
    
    save_results = params.get("save_topk_result")
    new_data, err_list = nnquery.get_topk_predictions_1(
        reranker,
        test_dataloader,
        candidate_pool_1,
        candidate_encoding,
    ...
    with open(os.path.join(params['output_path'], params["mode"] + "_err_list.txt"), "w") as f:
        # f.writelines("\n".join(err_list))
        for l in err_list:
            f.write(str(l) + "\n")
