...
class BiEncoderRanker(torch.nn.Module):
    def __init__(self, params, shared=None):
        ...
        if self.data_parallel:
            # self.model = torch.nn.DataParallel(self.model, device_ids=[0, 2, 3])
    ...
    def score_candidate(
        ...
        if len(cand_vecs.size()) == 3:
            bs, k, dim = cand_vecs.size()
            cand_vecs = cand_vecs.reshape(bs * k, dim)
        token_idx_cands, segment_idx_cands, mask_cands = to_bert_input(
        ...
        if random_negs:
            ...
            embedding_cands = embedding_cands.reshape(bs, k, -1)
            # train on hard negatives
            embedding_ctxt = embedding_ctxt.unsqueeze(1)  # batchsize x 1 x embed_size
            # embedding_cands = embedding_cands.unsqueeze(2)  # batchsize x embed_size x 2
            embedding_cands = embedding_cands.transpose(-1, -2) # batchsize x embed_size x k
    ...
    def forward(self, context_input, cand_input, negative_input=None, encode_only=False, encode_ctx_or_cand=False):
        if encode_only:
            if encode_ctx_or_cand:
                return self.encode_context(context_input)
            else:
                return self.encode_candidate(cand_input)
        else:
            flag = negative_input is None
            if negative_input is None:
                ...
                cand_input = cand_input.unsqueeze(1)
                if len(negative_input.size()) == 2:
                    negative_input = negative_input.unsqueeze(1)
                negative_with_label_input = torch.cat([cand_input, negative_input], dim=1)
                scores = self.score_candidate(context_input, negative_with_label_input, flag)
                bs = scores.size(0) # 当最后一个batch只有一条数据时会出问题
                target = torch.zeros(bs).long().to(self.device)
                # target = target.float()
                loss = F.cross_entropy(scores, target, reduction="mean")
            return loss, scores
