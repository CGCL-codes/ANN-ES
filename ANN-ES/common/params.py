# Provide an argument parser and default command line options for using ANN-ES.
import argparse
import importlib
import os
import sys
...
class ANNESParser(argparse.ArgumentParser):
    """
    ...
    def __init__(
        self, add_annes_args=True, add_model_args=False, 
        description='ANNES parser',
    ):
        super().__init__(
            ...
            add_help=add_annes_args,
        )
        self.annes_home = os.path.dirname(
            os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
        )
        os.environ['ANNES_HOME'] = self.annes_home
    ...
    def add_annes_args(self, args=None):
        """
    ...
    def add_model_args(self, args=None):
        ...
        parser.add_argument(
            ...
            type=str,
            help="The path to the documents data.",
        )
        parser.add_argument(
            "--output_path",
            default=None,
        ...
        parser.add_argument(
            "--hnsw",
            action="store_true",
            help="Whether to use faiss hnsw index. When none, use FlatIP index, else use hnsw index.",
        )
    ...
    def add_training_args(self, args=None):
        ...
        parser.add_argument(
            ...
            help="k nearest neighbor. k is the hard negative number",
        )
        parser.add_argument(
            "--encoding_batch_size", type=int, default=4096, 
            help="batch size for encoding context or candidates.",
        )
        parser.add_argument(
            "--exclude_glod",
            action="store_true",
            help="Whether to exclude gold when retrieve hard negatives. When none, not exclude, else exclude.",
        )
        parser.add_argument(
            "--summ_mode",
            type=str,
            default="None",
            choices=["None", "sumy", "presumm"],
            help="summ mode is in [None, sumy, presumm]. None indicates no summ nethod. Sumy indicates Word-Range method. Presumm indicates the bert-based method."
        )
    ...
    def add_eval_knn_args(self, args=None):
        """
        Add model evaluation args.
        """
        parser = self.add_argument_group("Model Evaluation Arguments")
